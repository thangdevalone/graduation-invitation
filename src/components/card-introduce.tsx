...
import { useStaggeredScrollReveal } from "@/lib/hooks";
import { BookOpen, GraduationCap, Heart, Star, Trophy } from "lucide-react";
import { motion } from "motion/react";

export function CardIntroduce() {
  // Scroll reveal with staggered animation for 5 grid items
  const {
    ref: cardsRef,
    isInView: areCardsInView,
    variants,
  } = useStaggeredScrollReveal<HTMLUListElement>(5, {
    threshold: 0.2,
  ...
  const gridItems = [
    {
      ...
      description:
        "Sau những năm tháng miệt mài học tập, chúng ta đã đến được đích đến quan trọng này.",
    },
    {
      ...
      description:
        "Mỗi thành tích đạt được đều là kết quả của sự nỗ lực không ngừng nghỉ.",
    },
    {
      ...
      description:
        "Những kiến thức đã học sẽ là nền tảng vững chắc cho hành trình phía trước.",
    },
    {
      ...
      description:
        "Với tấm bằng tốt nghiệp, cánh cửa cơ hội đang mở ra trước mắt.",
    },
    {
      ...
      description:
        "Lời cảm ơn chân thành đến gia đình, thầy cô và bạn bè đã luôn bên cạnh.",
    },
  ];

  return (
    <div className="max-w-7xl mx-auto md:px-8 lg:px-10 mb-10 md:mb-20 md:mt-30 my-20 px-4">
      <motion.ul
        ref={cardsRef}
